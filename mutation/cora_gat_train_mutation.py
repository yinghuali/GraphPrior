...
def train(x, edge_index, y, save_model_name, hidden_channels, dic):
    ...
    model = GAT(num_node_features, hidden_channels, num_classes, dic).to(device)
    x = x.to(device)
...
if __name__ == '__main__':
    list_dic = list(ParameterGrid(dic_mutation_gat))
    j = 0
    for i in hidden_channel_list:
        for dic in list_dic:
            save_model_name = 'mutation_models/cora_gat/cora_gat_' + str(i) + '_' + str(j) + '.pt'
            pickle.dump(dic, open('/Users/yinghua.li/Documents/Pycharm/GNNEST/mutation/mutation_models/cora_gat/cora_gat_' + str(i) + '_' + str(j) + '.pkl', 'wb'))
