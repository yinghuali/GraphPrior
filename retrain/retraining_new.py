...
def get_retrain(rank_list, check):
    all_res = []
    if check not in ['DeepGini', 'Entropy', 'LeastConfidence', 'Margin', 'VanillaSM', 'PCS', 'Random']:
        epochs=7
    else:
        epochs=5
...
def main():
    ...
    lr_np = get_retrain(lr_rank_idx, 'LRGP')
    lgb_np = get_retrain(lgb_rank_idx, 'LRGP')
    xgb_np = get_retrain(xgb_rank_idx, 'XGGP')
    dnn_np = get_retrain(dnn_rank_idx, 'DNGP')
    kmgp_np = get_retrain(mutation_rank_idx, 'KMGP')

    rf_np = get_retrain(rf_rank_idx, 'RFGP')
    margin_np = get_retrain(margin_rank_idx, 'Margin')
    deepGini_np = get_retrain(deepGini_rank_idx, 'DeepGini')
    random_np = get_retrain(random_rank_idx, 'Random')
    entropy_np = get_retrain(entropy_rank_idx, 'Entropy')
    leastConfidence_np = get_retrain(leastConfidence_rank_idx, 'LeastConfidence')
    vanillasoftmax_np = get_retrain(vanillasoftmax_rank_idx, 'VanillaSM')
    pcs_np = get_retrain(pcs_rank_idx, 'PCS')
    dic = {
        'KMGP': kmgp_np,
        'DNGP': dnn_np,
        'LGGP': lgb_np,
